...
class Train_Manager:
    ...
    def train(self,model):
        ...
        for e in tqdm(range(total_epoch)):
            # 更新以接收新的返回值
            iter_counter,train_acc,con_loss_value,contra_weight_value = train_func(model=model,
                                                optimizer=optimizer,
                                                writer=writer,
            ...
            if (e+1)%args.val_epoch==0:
                ...
                logger.info("contrastive_loss: %.3f" % (con_loss_value))
                logger.info("contrastive_weight: %.3f" % (contra_weight_value))  # 添加權重輸出
    ...
    def evaluate(self,model):
        ...
        with torch.no_grad():
            ...
            # Output best model parameters, now using current_contra_weight
            logger.info('Best model parameters:')
            logger.info('Current Contrastive Loss Weight: %.4f' % model.current_contra_weight)
            logger.info('W1: %.4f' % model.w1.item())
            logger.info('W2: %.4f' % model.w2.item())
            logger.info('Scale: %.4f' % model.scale.item())
